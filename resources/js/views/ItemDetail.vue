<template>
    <div
        class="relative w-full border-b-2 border-black bg-gray-softest"
        v-if="item"
    >
        <ItemImageMovable
    ...
    <div class="relative h-full border-black px-4 pb-24 pt-8" v-if="item">
        <div
            class="absolute -top-8 inline-block rounded-md bg-black p-1.5"
            v-if="item.code"
        >
            <img
                class="h-9 w-9"
                :src="`/img/${item.code}.svg`"
                :alt="item.code"
            />
        </div>
        ...
        <div class="p-4 pt-8">
            <div class="pointer-events-auto flex space-x-4">
                ...
                <ConfirmButton
                    v-else
                    class="bg-green"
                    @click="addItemFavourited(item)"
                >
                    <svg
                        class="mr-3 h-[22px] w-[25px] fill-green stroke-black stroke-2"
                    >
                        <path
